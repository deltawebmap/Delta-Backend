...
namespace ArkWebMapMasterServer.Services.Auth.NewAuth
{
    public class NewAuthBeginRequest : INewAuthService
    {
        ...
        public override async Task OnRequest()
        {
            ...
            string return_url = $"{conn.hosts.master}/api/auth/authenticate?{URLPARAM_TOKEN}={s.session_token}&{URLPARAM_NONCE}={HttpUtility.UrlEncode(r.nonce)}&{URLPARAM_RETRY}={HttpUtility.UrlEncode(JsonConvert.SerializeObject(retryData))}";
