...
{
    public class GenericGatewayClient
    {
        ...
        /// <returns></returns>
        public void InternalSetupClient(GatewayClientType type, string client_name, string client_name_extra, int client_version_major, int client_version_minor, bool logging_enabled, GatewayMessageHandler handler, string token)
        {
            //Create object and add args
            this.connectionType = type;
            this.client_name = client_name;
            this.client_name_extra = client_name_extra;
            this.client_version_major = client_version_major;
            this.client_version_minor = client_version_minor;
            this.logging_enabled = logging_enabled;
            this.handler = handler;
            this.txQueue = new Queue<byte[]>();
            this.token = token;
            ...
            try
            {
                using (WebClient wc = new WebClient())
                    configText = wc.DownloadString(REMOTE_CONFIG_ENDPOINT + CreateUrlParams());
            } catch (Exception ex)
            {
            ...
            this.TryMakeConnection().GetAwaiter().GetResult();

            //Create threads
            this.rxThread = new Thread(this.RxBgThread);
            this.rxThread.IsBackground = true;
            this.rxThread.Start();

            this.txThread = new Thread(this.TxBgThread);
            this.txThread.IsBackground = true;
            this.txThread.Start();
        }
        ...
        public string GetEndpointPathname()
        {
            ...
            switch(connectionType)
            {
                ...
                case GatewayClientType.Sender: return config.gateway_endpoints.sender;
            }
